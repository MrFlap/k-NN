...
public class RNNQueryFactory extends BaseQueryFactory {
    ...
    public static Query create(final RNNQueryFactory.CreateQueryRequest createQueryRequest) {
        ...
        // For quantized indices with MOS enabled and no decay: use bounded radial search.
        // This performs phase 1 (quantized ANN) + phase 2 (full-precision rescore of top ef_search)
        // in a single query, bounding full-precision work to O(min(ef_search, num_results)).
        if (createQueryRequest.isMemoryOptimizedSearchEnabled()
            && createQueryRequest.getVectorFieldType() != null
            && createQueryRequest.getVectorFieldType().isRescoringRequiredForRadial()
            && resolveDecay(createQueryRequest) == null) {

            final int efSearch = resolveEfSearch(createQueryRequest);
            final Query filterQuery = getFilterQuery(createQueryRequest);
            log.debug("Creating bounded radial query for index: {}, field: {}, radius: {}, efSearch: {}",
                createQueryRequest.getIndexName(), fieldName, radius, efSearch);
            return new BoundedRadialSearchQuery(fieldName, vector, radius, efSearch, filterQuery);
        }
